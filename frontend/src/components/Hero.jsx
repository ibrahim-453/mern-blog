...
// Helper component for the animated background blobs
const AnimatedBlob = ({ className }) => {
  return (
    <div
      className={`absolute w-72 h-72 rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-3xl animate-blob ${className}`}
    />
  );
};

// Helper component for the hero action buttons
const HeroButton = ({ to, icon: Icon, colorClasses, children }) => {
  const baseClasses =
    "group relative font-semibold text-sm md:text-base lg:text-lg text-white px-6 sm:px-7 md:px-8 lg:px-10 py-3 md:py-3.5 rounded-xl transition-all duration-300 text-center shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 overflow-hidden";
...
function Hero() {
  return (
    <main className="relative w-full min-h-[200px] md:min-h-[400px] bg-bg-primary dark:bg-bg-primary-dark duration-300 border-b-2 border-accent-1 dark:border-accent-1-dark overflow-hidden">
      {/* Animated background elements */}
      <div className="absolute inset-0 overflow-hidden opacity-30">
        <AnimatedBlob className="top-10 left-10 bg-accent-1 dark:bg-accent-1-dark" />
        <AnimatedBlob className="top-20 right-10 bg-accent-2 dark:bg-accent-2-dark [animation-delay:2s]" />
        <AnimatedBlob className="bottom-10 left-1/2 bg-accent-1 dark:bg-accent-1-dark [animation-delay:4s]" />
      </div>
      ...
      <div className="relative flex flex-col justify-center items-center gap-8 lg:gap-12 max-w-7xl mx-auto py-8 sm:py-12 md:py-20 lg:py-30 px-4">
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm sm:text-base md:text-lg text-text/70 dark:text-text-dark/70 text-center max-w-2xl mx-auto animate-fade-in [animation-delay:0.2s] fill-mode-both">
            Create, share, and discover amazing stories from writers around the
            world
          </p>
        </div>

        {/* Action Buttons */}
        <div className="flex flex-col sm:flex-row gap-4 md:gap-6 w-full sm:w-auto animate-fade-in-up [animation-delay:0.4s] fill-mode-both">
          <HeroButton
            to="write-blog"
            icon={PenTool}
            colorClasses="bg-accent-1 dark:bg-accent-1-dark hover:bg-hover dark:hover:bg-hover-dark"
          >
            Write A Blog
          </HeroButton>
          <HeroButton
            to="blogs"
            icon={BookOpen}
            colorClasses="bg-accent-2 dark:bg-accent-2-dark hover:bg-hover dark:hover:bg-hover-dark"
          >
            Read A Blog
          </HeroButton>
        </div>
